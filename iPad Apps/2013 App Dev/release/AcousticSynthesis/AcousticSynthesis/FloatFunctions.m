...
+(void)averageChunks:(float*)arr arraySize:(int)arrSize numChunks:(int)size output:(float*)averages{
    float length = (float)arrSize/size;
    float lower = 0;
    for(int i = 0; i<size; i++){
        float sum = 0;
        for(int j=round(lower);j<round(lower+length);j++){
            sum = sum + arr[j];
        }
        
        int avg = sum/(round(lower+length)-round(lower));
        averages[i] = avg;
        lower = lower + length;
    }
}
...
+(void) peaks:(float*)array numElements:(int)size{
    int previousVal = array[0];
    int goingUp = YES;
    for(int i = 1; i<size; i++){
        if(previousVal>array[i] && goingUp==YES)//peak found
        {
            previousVal = array[i];
            goingUp = NO;
        }
        ...
        if(previousVal<array[i] && goingUp==NO)
        {
            goingUp = YES;
        }
    }
