...
-(IBAction)freqSliderChanged:(id)sender {
    ...
    if(waveType==3)
    {
        indexStepSize = ((float)graphWidth/(kOutputSampleRate/baseFrequency));//shitty
    }
    else
    {
        indexStepSize = ((float)graphWidth/3/(kOutputSampleRate/baseFrequency));//shitty
    }
        ...
        if ([subView isKindOfClass:[UILabel class]]) {
             if ([subView tag] > 1000 && [subView tag] < 1010) {
                UILabel *tempLabel = (UILabel*)subView;
                 ...
                 if(waveType==3)
                 {
                      tempLabel.text = [NSString stringWithFormat:@"%.f",(1000*([subView tag]-1000))/baseFrequency];//shitty;
                 }
                 else
                 {
                     tempLabel.text = [NSString stringWithFormat:@"%.f",(1000*([subView tag]-1000))/baseFrequency*3];//shitty;
                 }
            }
        }
    }
    ...
    if (waveType!=3) {
        ...
        if (waveType == 0) {
            ...
            for (int i = 0;i<graphWidth;i++) {
                pointValues[i] = 0;
                for (int j = 1;j<=1*2;j+=2) {
                    pointValues[i] += (1.0/(float)j)*sinf(-j*6*M_PI*i/graphWidth);
                }
            }
            [FloatFunctions normalize:pointValues numElements:graphWidth];
            for (int i = 0;i<graphWidth;i++) {
                pointValues[i] = pointValues[i]*[ampSlider value]*(graphHeight/2.0-20)+graphHeight/2.0;
            }
            ...
            for (int i = 0;i<graphWidth;i++) {
                ...
                for (int j = 1;j<=kNumHarmonics;j++) {
                    pointValues[i] += (1.0/(float)j)*sinf(j*6*M_PI*(i-5)/(graphWidth-10));
                }
            }
            [FloatFunctions normalize:pointValues numElements:graphWidth];
            for (int i = 0;i<graphWidth;i++) {
