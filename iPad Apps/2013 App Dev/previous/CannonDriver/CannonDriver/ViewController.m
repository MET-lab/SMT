...
-(IBAction)waveTypeChanged:(id)sender {
    ...
    //NSLog(@"SelectedSegement: %d", waveType);
    //NSLog(@"fftCalled: %i", fftCalled);
    if (waveType == 0) {
        [self drawSineWave];
...
/* Functionality moved to a single button, named button
...
-(void) drawNoise {
    if (!audioDrawCalled) {
        // Can be used to "smooth out" the time domain plot, merely duplicates points a number of times
        int const SAMPLE_WIDTH = 2;
        for (int i = 0;i<audioDraw.frame.size.width;i += SAMPLE_WIDTH) {
            waveDrawValues[i] = randomFloat(20,audioDraw.frame.size.height-20);
            for (int j = i; j < i + SAMPLE_WIDTH; j++) {
                waveDrawValues[j] = waveDrawValues[i];
            }
        /* Orignal, one sample, one random number generator 
        for (int i = 0;i<audioDraw.frame.size.width;i += 2) {
            waveDrawValues[i] = randomFloat(20,audioDraw.frame.size.height-20);
         */
        }
...
-(void) drawFFT {
    ...
    //if (!isRunning) {
        ...
        fftCalled = NO;
    //}
    // NSLog(@"isRunning: %d, waveType: %d", isRunning, waveType);
    if (!fftCalled) {
